...
class Exact_equation:
    def __init__(self, x0, n, b):
        self.x0 = x0
        self.n = n
        self.b = b

    def array_exact(self):
        ...
        for i in range(1, self.n+1):
            ...
            if x[i] == 1:
                ...
                exact[i] = math.exp(x[i]) + 1/(1-x[i])

        return exact

    def array_x_axis(self):
...
class Eulers_method(Numerical_methods):
    ...
    def get_error(self):
        ...
        for i in range(1, self.n+1):
            error[i] = super().exact_solution(x[i]) - y[i]
...
class Draw_plot_for_exact_solution(Draw_plot):
    ...
    def plot(self):
        ...
        plt.plot(self.x, self.y1, marker='o',  markerfacecolor='darkblue',
                 markersize=5, color='skyblue', linewidth=2, label="exact solution")
...
class Draw_plot_for_maximal_local_error_plot(Draw_plot):
    ...
    def plot(self):
        ...
        plt.plot(self.x[self.initial_number_of_interval:self.finite_number_if_interval], self.y1[self.initial_number_of_interval:self.finite_number_if_interval],
                 marker='o',  markerfacecolor='blue', markersize=5, color='skyblue', linewidth=2, label="euler's method")
        plt.plot(self.x[self.initial_number_of_interval:self.finite_number_if_interval], self.y2[self.initial_number_of_interval:self.finite_number_if_interval],
                 marker='o',  markerfacecolor='green', markersize=5, color='darkgreen', linewidth=2, label="improved euler method")
        plt.plot(self.x[self.initial_number_of_interval:self.finite_number_if_interval], self.y3[self.initial_number_of_interval:self.finite_number_if_interval],
                 marker='o',  markerfacecolor='red', markersize=5, color='orange', linewidth=2, label="runge-kutta method")
        plt.title("Maximal local error plot", loc='left',
...
if __name__ == "__main__":
    ...
    exact_equation = Exact_equation(
        float(sys.argv[1]), int(sys.argv[4]), float(sys.argv[3]))


    plot_exact_solution = Draw_plot_for_exact_solution(
        exact_equation.array_x_axis(), exact_equation.array_exact())

    plot_local_error = Draw_plot_for_local_error_plot(exact_equation.array_x_axis(), euler_method.get_error(
    ), improved_euler_method.get_error(), runge_kutta_method.get_error())

    plot_maximal_local_error = Draw_plot_for_maximal_local_error_plot(exact_equation.array_x_axis(), euler_method.get_error(
    ), improved_euler_method.get_error(), runge_kutta_method.get_error(), int(sys.argv[5]), int(sys.argv[6]))


    plot_exact_solution.plot()
    plot_local_error.plot()
    plot_maximal_local_error.plot()
